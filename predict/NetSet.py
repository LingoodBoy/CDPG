import sys
from copy import deepcopy
from datetime import datetime

import numpy as np
import scipy.sparse as sp
import torch
from Models.graph_transformer import v_GraphTransformer
from torch import nn, optim
from torch.nn import functional as F
from tqdm import tqdm
from utils.train_utils import *

from utils.train_utils import set_random_seed
set_random_seed(seed=42)
device = get_device()

def smart_inverse_transform(normalized_data, dataset, node_index=0, feature_index=0):

    node_log_mean = dataset.node_log_means[node_index, feature_index]
    node_log_std = dataset.node_log_stds[node_index, feature_index]

    log_mean_tensor = torch.tensor(node_log_mean, dtype=normalized_data.dtype, device=normalized_data.device)
    log_std_tensor = torch.tensor(node_log_std, dtype=normalized_data.dtype, device=normalized_data.device)
    log_scale_data = normalized_data * log_std_tensor + log_mean_tensor

    return log_scale_data

def asym_adj(adj):
    adj = adj.cpu().numpy()
    adj = sp.coo_matrix(adj)
    rowsum = np.array(adj.sum(1)).flatten()
    d_inv = np.power(rowsum, -1).flatten()
    d_inv[np.isinf(d_inv)] = 0.
    d_mat= sp.diags(d_inv)
    return d_mat.dot(adj).astype(np.float32).todense()

class StgnnSet(nn.Module): 
    def __init__(self, data_args, task_args, model_args, model='GRU', node_num=207):
        super(StgnnSet, self).__init__()
        self.data_args = data_args
        self.task_args = task_args
        self.model_args = model_args
        
        self.update_lr = model_args['update_lr']
        self.meta_lr = model_args['meta_lr']
        self.update_step = model_args['update_step']
        self.update_step_test = model_args['update_step_test']
        self.task_num = task_args['task_num']
        self.model_name = model

        self.loss_lambda = model_args['loss_lambda']
        print("loss_lambda = ", self.loss_lambda)

        self.model = v_GraphTransformer(model_args, task_args)

        self.meta_optim = optim.Adam(self.model.parameters(), lr=self.meta_lr, weight_decay=1e-2)
        self.loss_criterion = nn.MSELoss()

    def graph_reconstruction_loss(self, meta_graph, adj_graph):
        adj_graph = adj_graph.unsqueeze(0).float()
        for i in range(meta_graph.shape[0]):
            if i == 0:
                matrix = adj_graph
            else:
                matrix = torch.cat((matrix, adj_graph), 0)
        criteria = nn.MSELoss()
        loss = criteria(meta_graph, matrix.float())
        return loss
    

    def calculate_loss(self, out, y, meta_graph, matrix, stage='target', graph_loss=True, loss_lambda=1):
        if loss_lambda == 0:
            loss = self.loss_criterion(out, y)
        if graph_loss and meta_graph is not None:  
            if stage == 'source' or stage == 'target_maml':
                loss_predict = self.loss_criterion(out, y)  
                loss_reconsturct = self.graph_reconstruction_loss(meta_graph, matrix)
            else:
                loss_predict = self.loss_criterion(out, y)
                loss_reconsturct = self.loss_criterion(meta_graph, matrix.float())
            loss = loss_predict + loss_lambda * loss_reconsturct
        else:
            loss = self.loss_criterion(out, y)

        return loss
    

    def forward(self, data, matrix):
        out, meta_graph = self.model(data, matrix)
        return out, meta_graph

    def finetuning(self, target_dataloader, test_dataloader, target_epochs,logger):

        maml_model = deepcopy(self.model)

        optimizer = optim.Adam(maml_model.parameters(), lr=self.meta_lr, weight_decay=1e-2)
        min_MAE = 10000000
        best_result = ''
        best_meta_graph = -1

        for epoch in tqdm(range(target_epochs)):
            train_losses = []
            start_time = time.time()
            maml_model.train()

            for step, (data, A_wave) in enumerate(target_dataloader):
                data, A_wave = data.to(device), A_wave.to(device)
                data.node_num = data.node_num[0]

                batch_size, node_num, seq_len, _ = data.x.shape

                out, meta_graph = maml_model(data, A_wave[0].float())
                loss = self.calculate_loss(out, data.y, meta_graph, A_wave, 'test', loss_lambda=self.loss_lambda)
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                train_losses.append(loss.detach().cpu().numpy())
            avg_train_loss = sum(train_losses)/len(train_losses)
            end_time = time.time()
            if epoch % 5 == 0:
                logger.info("[Target Fine-tune] epoch #{}/{}: loss is {}, fine-tuning time is {}".format(epoch+1, target_epochs, avg_train_loss, end_time-start_time))

        with torch.no_grad():
            test_start = time.time()
            maml_model.eval()
            for step, (data, A_wave) in enumerate(test_dataloader):
                data, A_wave = data.to(device), A_wave.to(device)
                data.node_num = data.node_num[0]
                batch_size, node_num, seq_len, _ = data.x.shape

                out, meta_graph = maml_model(data, A_wave[0].float())

                if step == 0:
                    outputs = out
                    y_label = data.y
                else:
                    outputs = torch.cat((outputs, out))
                    y_label = torch.cat((y_label, data.y))
            outputs = outputs.permute(0, 2, 1).detach().cpu().numpy()
            y_label = y_label.permute(0, 2, 1).detach().cpu().numpy()
            result = metric_func(pred=outputs, y=y_label, times=self.task_args['pred_num'])
            test_end = time.time()

            result_print(result, logger,info_name='Evaluate')
            logger.info("[Target Test] testing time is {}".format(test_end-test_start))

    def taskTrain(self, taskmode, target_dataloader, test_dataloader, target_epochs, logger, meanstd):

        optimizer = optim.Adam(self.model.parameters(), lr=self.meta_lr, weight_decay=1e-2)
        min_MAE = 10000000
        best_result = ''
        best_meta_graph = -1

        for epoch in tqdm(range(target_epochs)):
            train_losses = []
            start_time = time.time()
            self.model.train()

            for step, (data, A_wave) in enumerate(target_dataloader):
                data, A_wave = data.to(device), A_wave.to(device)
                data.node_num = data.node_num[0]

                batch_size, node_num, seq_len, _ = data.x.shape

                out, meta_graph = self.model(data, A_wave[0].float())

                out = out * meanstd[1] + meanstd[0]
                gold = data.y * meanstd[1] + meanstd[0]

                loss = self.loss_criterion(out, gold)

                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                train_losses.append(loss.detach().cpu().numpy())
            avg_train_loss = sum(train_losses)/len(train_losses)
            end_time = time.time()
            if epoch % 5 == 0:
                logger.info("[Target Fine-tune] epoch #{}/{}: loss is {}".format(epoch+1, target_epochs, avg_train_loss))
                if taskmode=='task3':
                    torch.save(self.model,'Param/{}_inside.pt'.format(taskmode))

        with torch.no_grad():
            self.model.eval()
            for step, (data, A_wave) in enumerate(test_dataloader):
                data, A_wave = data.to(device), A_wave.to(device)
                data.node_num = data.node_num[0]
                batch_size, node_num, seq_len, _ = data.x.shape

                out, meta_graph = self.model(data, A_wave[0].float())
                
                out = out * meanstd[1] + meanstd[0]
                gold = data.y * meanstd[1] + meanstd[0]

                if step == 0:
                    outputs = out
                    y_label = gold
                else:
                    outputs = torch.cat((outputs, out))
                    y_label = torch.cat((y_label, gold))
            outputs = outputs.permute(0, 2, 1).detach().cpu().numpy()
            y_label = y_label.permute(0, 2, 1).detach().cpu().numpy()
            result = metric_func(pred=outputs, y=y_label, times=self.task_args['pred_num'])

            result_print(result, logger,info_name='Evaluate')


    
    def node_taskTrain(self, modelname, node_index, init_index, target_dataloader, test_dataloader, train_dataset, test_dataset, target_epochs,logger, test_dataset_name, ifnewname, aftername, writer=None, timestamp=None, node_range=None):

        if ifnewname == 1:
            if timestamp:
                if node_range:
                    rootpath = 'Param/pretrain/{}_{}_{}_{}_{}_{}/' .format(test_dataset_name, modelname, node_range, aftername, timestamp)
                else:
                    rootpath = 'Param/pretrain/{}_{}_{}_{}/' .format(test_dataset_name, modelname, aftername, timestamp)
            else:
                if node_range:
                    rootpath = 'Param/pretrain/{}_{}_{}_{}_{}/' .format(test_dataset_name, modelname, node_range, aftername)
                else:
                    rootpath = 'Param/pretrain/{}_{}_{}/' .format(test_dataset_name, modelname, aftername)
        elif ifnewname == 2: 
            if timestamp:
                if node_range:
                    rootpath = 'Param/pretrain/{}_{}_{}_{}_{}/' .format(test_dataset_name, modelname, node_range, aftername, timestamp)
                else:
                    rootpath = 'Param/pretrain/{}_{}_{}_{}/' .format(test_dataset_name, modelname, aftername, timestamp)
            else:
                import datetime
                current_timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                if node_range:
                    rootpath = 'Param/pretrain/{}_{}_{}_{}_{}/' .format(test_dataset_name, modelname, node_range, aftername, current_timestamp)
                else:
                    rootpath = 'Param/pretrain/{}_{}_{}_{}/' .format(test_dataset_name, modelname, aftername, current_timestamp)
        else:
            if timestamp:
                if node_range:
                    rootpath = 'Param/pretrain/{}_{}_{}_{}_{}/' .format(test_dataset_name, modelname, node_range, aftername, timestamp)
                else:
                    rootpath = 'Param/pretrain/{}_{}_{}_{}/' .format(test_dataset_name, modelname, aftername, timestamp)
            else:
                if node_range:
                    rootpath = 'Param/pretrain/{}_{}_{}/' .format(test_dataset_name, modelname, node_range)
                else:
                    rootpath = 'Param/pretrain/{}_{}/' .format(test_dataset_name, modelname)

        folder = os.path.exists(rootpath + 'Node_{}'.format(init_index))
        if not folder:
            os.makedirs(rootpath + 'Node_{}' .format(init_index))

        optimizer = optim.AdamW(
            self.model.parameters(),
            lr=self.meta_lr,
            weight_decay=0.01,
            betas=(0.9, 0.999)
        )

        mintestloss = 9999999 
        bestmodel = deepcopy(self.model)
        badtestres = 0 

        for epoch in tqdm(range(target_epochs)):
            train_losses = []
            self.model.train()

            for step, (data, A_wave) in enumerate(target_dataloader):

                data, A_wave = data.to(device), A_wave.to(device)

                out, _ = self.model(data, A_wave[0].float())

                outputs = out[:,node_index,:] 
                y_label = data.y[:,node_index,:] 

                loss = self.loss_criterion(outputs, y_label) 

                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                train_losses.append(loss.detach().cpu().numpy())

            avg_train_loss = sum(train_losses)/len(train_losses)

            if writer is not None:
                global_step = init_index * target_epochs + epoch
                writer.add_scalar(f'Node_{init_index}/Train_Loss', avg_train_loss, epoch)

            if epoch > 0 and epoch % 2 == 0:
                with torch.no_grad():
                    self.model.eval()

                    val_losses = []  

                    for step, (data, A_wave) in enumerate(test_dataloader):
                        data, A_wave = data.to(device), A_wave.to(device)

                        out, meta_graph = self.model(data, A_wave[0].float())

                        out_normalized = out[:,node_index,:] 
                        datay_normalized = data.y[:,node_index,:] 

                        loss = self.loss_criterion(out_normalized, datay_normalized)
                        val_losses.append(loss.detach().cpu().numpy())

                        out = smart_inverse_transform(out[:,node_index,:], train_dataset, node_index, 0) 
                        datay = smart_inverse_transform(data.y[:,node_index,:], train_dataset, node_index, 0) 

                        if step == 0:
                            outputs = out.unsqueeze(1)
                            y_label = datay.unsqueeze(1) 
                        else:

                            outputs = torch.cat((outputs, out.unsqueeze(1)),dim=1)
                            y_label = torch.cat((y_label, datay.unsqueeze(1)),dim=1)
                
                    avg_val_loss = sum(val_losses)/len(val_losses) if val_losses else float('inf')

                    if avg_val_loss < mintestloss:
                        bestmodel = deepcopy(self.model)
                        mintestloss = avg_val_loss
                        badtestres = 0
                    else:
                        badtestres += 1

                        if badtestres >= 8:
                            logger.info(f"Early stopping at epoch {epoch}: -- {badtestres * 2} epoch not improved")
                            break

                    if writer is not None:
                        writer.add_scalar(f'Node_{init_index}/Val_Loss', avg_val_loss, epoch)
                        writer.add_scalar(f'Node_{init_index}/Learning_Rate', optimizer.param_groups[0]['lr'], epoch)

        torch.save(bestmodel,rootpath + 'node_{}.pt' .format(init_index))

        with torch.no_grad():
            self.model = torch.load(rootpath + 'node_{}.pt' .format(init_index))
            self.model.eval()

            for step, (data, A_wave) in enumerate(test_dataloader):
                data, A_wave = data.to(device), A_wave.to(device)
                data.node_num = data.node_num[0]
                batch_size, node_num, seq_len, _ = data.x.shape

                out, meta_graph = self.model(data, A_wave[0].float())

                out = smart_inverse_transform(out[:,node_index,:], train_dataset, node_index, 0) 
                datay = smart_inverse_transform(data.y[:,node_index,:], train_dataset, node_index, 0) 
                
                if step == 0:
                    outputs = out.unsqueeze(1) 
                    y_label = datay.unsqueeze(1) 
                else:

                    outputs = torch.cat((outputs, out.unsqueeze(1)),dim=1) 
                    y_label = torch.cat((y_label, datay.unsqueeze(1)),dim=1) 

            outputs = outputs.permute(0, 2, 1).detach().cpu().numpy()  
            y_label = y_label.permute(0, 2, 1).detach().cpu().numpy()  
            result = metric_func(pred=outputs, y=y_label, times=self.task_args['pred_num'])

            result_print(result, logger,info_name='Evaluate')

            return outputs, y_label
    
    def node_eval(self, node_index, test_dataloader, logger, test_dataset):

        with torch.no_grad():
            self.model = torch.load('Param/Task3_1/{}/task3_{}.pt' .format(test_dataset, node_index))
            test_start = time.time() 
            self.model.eval()
            for step, (data, A_wave) in enumerate(test_dataloader):

                A_wave = A_wave[0][node_index,:].unsqueeze(0)
                data, A_wave = data.to(device), A_wave.to(device)
                data.node_num = data.node_num[0]
                batch_size, node_num, seq_len, _ = data.x.shape

                out, meta_graph = self.model(data, A_wave.float())

                if step == 0:
                    outputs = out
                    y_label = data.y
                else:
                    outputs = torch.cat((outputs, out))
                    y_label = torch.cat((y_label, data.y))
            outputs = outputs.permute(0, 2, 1).detach().cpu().numpy()
            y_label = y_label.permute(0, 2, 1).detach().cpu().numpy()
            result = metric_func(pred=outputs, y=y_label, times=self.task_args['pred_num'])
            test_end = time.time()

            result_print(result, logger,info_name='Evaluate')
            logger.info("[Test] testing time is {}".format(test_end-test_start))

            return outputs, y_label
    
    def taskEval(self, test_dataloader, logger):
        self.model = torch.load('Param/task1.pt')
        with torch.no_grad():
            test_start = time.time() 
            self.model.eval()
            for step, (data, A_wave) in enumerate(test_dataloader):
                data, A_wave = data.to(device), A_wave.to(device)
                data.node_num = data.node_num[0]
                batch_size, node_num, seq_len, _ = data.x.shape

                out, meta_graph = self.model(data, A_wave[0].float())

                if step == 0:
                    outputs = out
                    y_label = data.y
                else:
                    outputs = torch.cat((outputs, out))
                    y_label = torch.cat((y_label, data.y))
            outputs = outputs.permute(0, 2, 1).detach().cpu().numpy()
            y_label = y_label.permute(0, 2, 1).detach().cpu().numpy()

            result = metric_func(pred=outputs, y=y_label, times=self.task_args['pred_num'])
            test_end = time.time()

            result_print(result, logger,info_name='Evaluate')
            logger.info("[Target Test] testing time is {}".format(test_end-test_start))


    def task5(self, taskmode, target_dataloader, test_dataloader, target_epochs,logger):

        optimizer = optim.Adam(self.model.parameters(), lr=self.meta_lr, weight_decay=1e-2)
        min_MAE = 10000000
        best_result = ''
        best_meta_graph = -1

        for epoch in tqdm(range(target_epochs)):
            train_losses = []
            start_time = time.time()
            self.model.train()

            for step, (data, A_wave) in enumerate(target_dataloader):
                data, A_wave = data.to(device), A_wave.to(device)
                data.node_num = data.node_num[0]

                batch_size, node_num, seq_len, _ = data.x.shape

                out, meta_graph = self.model(data, A_wave[0].float())
                loss = self.calculate_loss(out, data.y, meta_graph, A_wave, 'test', loss_lambda=self.loss_lambda)
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                train_losses.append(loss.detach().cpu().numpy())
            avg_train_loss = sum(train_losses)/len(train_losses)
            end_time = time.time()
            if epoch % 5 == 0:
                logger.info("[Target Fine-tune] epoch #{}/{}: loss is {}, fine-tuning time is {}".format(epoch+1, target_epochs, avg_train_loss, end_time-start_time))
                if taskmode=='task3':
                    torch.save(self.model,'Param/{}_inside.pt'.format(taskmode))
        
        torch.save(self.model,'Param/{}_inside.pt'.format(taskmode))

        with torch.no_grad():
            test_start = time.time() 
            self.model.eval()
            for step, (data, A_wave) in enumerate(test_dataloader):
                data, A_wave = data.to(device), A_wave.to(device)
                data.node_num = data.node_num[0]
                batch_size, node_num, seq_len, _ = data.x.shape

                out, meta_graph = self.model(data, A_wave[0].float())

                if step == 0:
                    outputs = out
                    y_label = data.y
                else:
                    outputs = torch.cat((outputs, out))
                    y_label = torch.cat((y_label, data.y))
            outputs = outputs.permute(0, 2, 1).detach().cpu().numpy()
            y_label = y_label.permute(0, 2, 1).detach().cpu().numpy()
            result = metric_func(pred=outputs, y=y_label, times=self.task_args['pred_num'])
            test_end = time.time()

            result_print(result, logger,info_name='Evaluate')
            logger.info("[Target Test] testing time is {}".format(test_end-test_start))

    def node_taskTrain2(self, param, modelname, node_index, init_index, target_dataloader, test_dataloader, train_dataset, test_dataset, target_epochs,logger, test_dataset_name, ifnewname, aftername, node_range=None):

        indexstart = [
            0, 192, 208, 464, 480, 736, 752, 1008,
            1024, 1280, 1296, 1552, 1568, 1824, 1840, 2096,
            2112, 2368, 2384, 2400, 2416, 2432, 2448, 2464,
            2480, 2992, 3024, 3536, 3552, 6624, 6640,
        ]
        shapes = [
            (12, 16), (16, 1), (16, 16), (16,),
            (16, 16), (16,), (16, 16), (16,),
            (16, 16), (16,), (16, 16), (16,),
            (16, 16), (16,), (16, 16), (16,),
            (16, 16), (16,), (16,), (16,),
            (16,), (16,), (16,), (16,),
            (32, 16), (32,), (16, 32), (16,),
            (16, 192), (1, 16),
        ]
        index = 0
        for key in self.model.state_dict().keys():
            # print(key)
            if ('running_mean' not in key) and ('running_var' not in key) and ('num_batches_tracked' not in key):

                pa = torch.tensor(param[indexstart[index]:indexstart[index+1]], device=device)
                pa = torch.reshape(pa, shapes[index])
                self.model.state_dict()[key].copy_(pa)
                index = index + 1
        print('params load ok')

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

        if ifnewname == 1:
            if node_range:
                rootpath = 'Param/finetune/{}_{}_{}_{}/' .format(test_dataset_name, modelname, node_range, aftername)
            else:
                rootpath = 'Param/finetune/{}_{}_{}_{}/' .format(test_dataset_name, modelname, aftername, timestamp)
        else:
            if node_range:
                rootpath = 'Param/finetune/{}_{}_{}_{}/' .format(test_dataset_name, modelname, node_range, timestamp)
            else:
                rootpath = 'Param/finetune/{}_{}_{}/' .format(test_dataset_name, modelname, timestamp)
        folder = os.path.exists(rootpath + 'Node_{}'.format(init_index))
        if not folder:
            os.makedirs(rootpath + 'Node_{}' .format(init_index)) 

        optimizer = optim.AdamW(
            self.model.parameters(),
            lr=self.meta_lr,
            weight_decay=0.01,
            betas=(0.9, 0.999)
        )

        mintestloss = 9999999
        bestmodel = deepcopy(self.model)
        badtestres = 0

        for epoch in tqdm(range(target_epochs)):
            train_losses = []
            self.model.train()

            for step, (data, A_wave) in enumerate(target_dataloader):
                data, A_wave = data.to(device), A_wave.to(device)

                out, _ = self.model(data, A_wave[0].float())
                
                outputs = smart_inverse_transform(out[:,node_index,:], train_dataset, node_index, 0)
                y_label = smart_inverse_transform(data.y[:,node_index,:], train_dataset, node_index, 0)
                loss = self.loss_criterion(outputs, y_label) 

                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                train_losses.append(loss.detach().cpu().numpy())
            avg_train_loss = sum(train_losses)/len(train_losses)


            if epoch % 2 == 0:
                with torch.no_grad():  
                    self.model.eval()
                    
                    test_losses = []
                    for step, (data, A_wave) in enumerate(test_dataloader):
                        data, A_wave = data.to(device), A_wave.to(device)

                        out, meta_graph = self.model(data, A_wave[0].float())

                        loss = self.loss_criterion(out[:,node_index,:], data.y[:,node_index,:])
                        test_losses.append(loss.detach().cpu().numpy())

                        out = smart_inverse_transform(out[:,node_index,:], train_dataset, node_index, 0)
                        datay = smart_inverse_transform(data.y[:,node_index,:], train_dataset, node_index, 0)

                        if step == 0:
                            outputs = out.unsqueeze(1)
                            y_label = datay.unsqueeze(1)
                        else:
                            outputs = torch.cat((outputs, out.unsqueeze(1)),dim=1)
                            y_label = torch.cat((y_label, datay.unsqueeze(1)),dim=1)

                    avg_test_loss = sum(test_losses)/len(test_losses)

                    outputs = outputs.permute(0, 2, 1).detach().cpu().numpy()
                    y_label = y_label.permute(0, 2, 1).detach().cpu().numpy()

                    result = metric_func(pred=outputs, y=y_label, times=self.task_args['pred_num'])


                    if avg_test_loss<mintestloss:
                        bestmodel = deepcopy(self.model)
                        mintestloss = avg_test_loss
                        badtestres = 0
                    else:
                        badtestres = badtestres + 1

                        if badtestres >= 8:
                            logger.info(f"Early stop at epoch {epoch}: with {badtestres * 2} no improvement")
                            break

                    total_RMSE, total_MAE = result['RMSE'], result['MAE']

                    lossmetric = [epoch, avg_train_loss, avg_test_loss]
                    lossmetric.extend(total_MAE.tolist()) 
                    lossmetric.extend(total_RMSE.tolist()) 

                    with open(rootpath + 'Node_{}/NodeTrain_{}_LossAndMetric.txt' .format(init_index, init_index), 'a') as f:
                        f.write(" ".join(str(i) for i in lossmetric) + '\n')
        with torch.no_grad():
            self.model = bestmodel
            self.model.eval()
            for step, (data, A_wave) in enumerate(test_dataloader):
                data, A_wave = data.to(device), A_wave.to(device)
                data.node_num = data.node_num[0]
                batch_size, node_num, seq_len, _ = data.x.shape

                out, meta_graph = self.model(data, A_wave[0].float())
                
                out = smart_inverse_transform(out[:,node_index,:], train_dataset, node_index, 0)
                datay = smart_inverse_transform(data.y[:,node_index,:], train_dataset, node_index, 0)
                
                if step == 0:
                    outputs = out.unsqueeze(1)
                    y_label = datay.unsqueeze(1)
                else:
                    outputs = torch.cat((outputs, out.unsqueeze(1)),dim=1)
                    y_label = torch.cat((y_label, datay.unsqueeze(1)),dim=1)

            outputs = outputs.permute(0, 2, 1).detach().cpu().numpy()
            y_label = y_label.permute(0, 2, 1).detach().cpu().numpy()
            result = metric_func(pred=outputs, y=y_label, times=self.task_args['pred_num'])

            result_print(result, logger,info_name='Evaluate')
            return outputs, y_label
